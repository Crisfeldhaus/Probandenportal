...
router.get("/admin/test/:23",authUser ,authRole, (req, res) => {
 
    res.render("admin/termin/test")
  
});

router.get("/admin/termine/bearbeiten/:id",authUser, authRole,(req,res) => {
  ...
  Termin.findByPk(id).then(termin1 => {
    if(termin1 != undefined){
      res.render("admin/termin/bearbeiten",{termines: termin1});
    }else{
      res.redirect("/admin/termine");
    }
  }).catch(erro => {
